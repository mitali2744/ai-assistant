...
def add_flashcard(question, answer):
    question, answer = question.strip(), answer.strip()  # remove whitespace
    if not question or not answer:
        return "Please provide both a question and answer."
    conn = get_connection()
    init_flashcards(conn)   # ensure table exists before inserting
...
def show_flashcards():
    ...
    for r in rows:
        acc = str(r[3]) + "/" + str(r[4]) if r[4] > 0 else "untested"  # correct/attempts ratio
...
# in-memory dict to track the currently active quiz question
...
def start_quiz():
    ...
    card = random.choice(rows)   # pick a random card from all flashcards
...
def check_answer(user_answer):
    ...
    conn = get_connection()
    # partial match: correct if either string contains the other
    is_correct = correct_answer in user_answer or user_answer in correct_answer
    # update attempts and correct count in DB
    conn.execute(
    ...
    conn.commit()
    conn.close()
    _quiz_state["active"] = False   # deactivate quiz after answer
...
def is_quiz_active():
    # return True if a quiz question is currently waiting for an answer
