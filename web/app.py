import sys
import os
# add project root to Python path so imports like "from core.x import y" work
...
app = Flask(__name__)
# secret key used to sign session cookies — loaded from env var in production
app.secret_key = os.environ.get("SECRET_KEY", "aria_secret_key_2026")

set_speak_callback(lambda text: None)  # disable voice in web mode

@app.before_request
def ensure_db():
    # run init_db before every request — needed on Vercel where DB may not exist yet
    try:
        init_db()
    except Exception:
        pass   # silently ignore if DB can't be created (e.g. read-only filesystem)

def get_user_id():
    # get logged-in user's ID from session, default to 1 if not set
...
def login():
    # handles both login and register using a hidden 'mode' form field
    mode    = request.form.get("mode", "login")  # default to login mode
    message = None
    error   = False
    ...
    if request.method == "POST" and "username" in request.form:
        ...
        if mode == "register":
            ...
            if existing:
                ...
                pw_hash = generate_password_hash(password)   # hash password before storing
                conn.execute("INSERT INTO users (username, password_hash) VALUES (?, ?)", (username, pw_hash))
                conn.commit()
                conn.close()
                message = f"Account created! Welcome, {username}. Please login."
                mode    = "login"   # switch to login tab after registration
        else:
            conn = get_connection()
...
@app.route("/logout")
def logout():
    session.clear()   # remove all session data (logs user out)
...
def chat():
    ...
    user_msg = request.json.get("message", "").strip()   # get message from JSON body
    if not user_msg:
        return jsonify({"type": "text", "response": "Please type something."})
    user_id  = get_user_id()
    response = process_query(user_msg, user_id=user_id)  # pass to assistant brain
    ...
    if response and response.startswith("IMAGE:"):
        return jsonify({"type": "image", "image": response[6:]})  # strip "IMAGE:" prefix
    return jsonify({"type": "text", "response": response})

@app.route("/status")
def status():
    # returns sidebar data: task counts, due today, XP status
    if "user_id" not in session:
